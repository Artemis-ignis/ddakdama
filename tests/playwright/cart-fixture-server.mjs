...
createServer(async(req,res)=>{
 ...
 if(req.method==="GET"&&url.pathname==="/cart"){const rows=[...products.values()].flatMap(product=>{const quantity=quantities.get(key(product))??0;if(!quantity)return[];return[`<div id="item_${product.productId}" data-vid="${product.vendorItemId}" data-line-total="${(product.price??0)*quantity}"><a href="https://www.coupang.com/vp/products/${product.productId}?vendorItemId=${product.vendorItemId}&sourceType=CART">${product.title}</a><input class="cart-quantity-input" type="text" value="${quantity}"><span>${(product.price??0)*quantity}원</span></div>`]});return html(res,`<!doctype html><html><head><meta charset="utf-8"></head><body>${rows.join("")}</body></html>`)}
