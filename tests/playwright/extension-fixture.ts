...
const liveCoupang=process.env.DDAKDAMA_LIVE_COUPANG==="1";
const sourceBuildPath=liveCoupang?path.join(sourceExtensionPath,"dist"):path.resolve(here,"../../output/extension-test-dist");
...
export const test=base.extend<{context:BrowserContext;extensionId:string;extensionWorker:Worker}>({
 context:async({},use)=>{prepareExtension();const context=await chromium.launchPersistentContext("",{channel:"chromium",headless:!liveCoupang,args:[`--disable-extensions-except=${extensionPath}`,`--load-extension=${extensionPath}`]});try{await use(context)}finally{await context.close();rmSync(temporaryRoot,{recursive:true,force:true})}},
