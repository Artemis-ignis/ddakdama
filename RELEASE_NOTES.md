...
- Vitest 58/58 통과
- 실제 Chromium Playwright 28/28 통과
- MCP 실연결 스모크 테스트 통과
- 프로덕션 빌드 통과
- 알려진 고위험 의존성 취약점 0개
...
- GPT 위젯 내부 페어링·전송·상태·해제 도구를 앱 전용으로 숨기고 위젯 리소스 URI를 v6로 갱신
- 기존 대화가 캐시한 v5 위젯 URI도 최신 v6 위젯 내용으로 읽을 수 있게 등록해 `resources/read` 실패를 방지
- GPT 위젯의 연결 grant를 안전한 호스트 상태와 로컬 앱 저장소에 복원하고, 새 계획을 연결된 확장 프로그램으로 자동 전송
- ChatGPT 테마와 운영체제 색상 설정을 따르는 라이트·다크 디자인 토큰 적용

고정 HTTPS Worker와 공개 `/mcp`는 `https://ddakdama.ddakdama.workers.dev`에 배포되어 실연결 검증을 통과했습니다. 기존 ChatGPT 앱의 Server URL 교체, 승인된 쿠팡 파트너스 키 호출과 실제 쿠팡 장바구니 변경은 계정·권한 및 명시적 승인 후 최종 검증해야 합니다.
