...
const tokenize=(text:string):Token[]=>[...text.matchAll(/(\d+(?:\.\d+)?)\s*(mL|ml|L|l|kg|g|mg|mcg|μg|IU|iu|%|정|캡슐|포|매|개입|스틱|패치|개|병|통|세트|박스|입)(?=\s|$)/gu)].map(m=>({value:Number(m[1]),unit:m[2],start:m.index??0}));
...
export function parseShoppingLine(rawText:string,index=0):ShoppingRequestLine{
  ...
  const multiplier=normalizedText.match(/(?:^|\s)[x×]\s*(\d+)\s*$/iu); if(multiplier)requestedPhysicalUnits=Number(multiplier[1]);
  const nameEnd=tokens.length?Math.min(...tokens.map(t=>t.start)):normalizedText.length;
