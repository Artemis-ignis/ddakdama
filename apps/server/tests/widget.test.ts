...
describe("ChatGPT 앱 위젯 계약", () => {
  it("내장 위젯 스크립트가 실행 가능한 JavaScript 문법이다", () => {
    const script = widgetHtml.match(/<script type="module">([\s\S]*?)<\/script>/)?.[1];
  ...
  it("재시도할 때 같은 idempotency key를 사용하고 수신 확인을 제공한다", () => {
  ...
  it("일반 사용자 화면에는 개발자 연결 정보를 노출하지 않는다", () => {
    ...
    for (const forbidden of ["MCP", "서버 주소", "server origin", "token", "handoff ID", "API endpoint", "Secret Key"]) {
