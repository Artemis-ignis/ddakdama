...
createServer(async(req,res)=>{try{const url=new URL(req.url??"/",`http://${req.headers.host??"localhost"}`);
 ...
 const ack=url.pathname.match(/^\/api\/handoffs\/([^/]+)\/ack$/);if(req.method==="POST"&&ack){const deviceId=authenticateDevice(bearer(req));if(!deviceId)return json(req,res,401,{error:"unauthorized"});return json(req,res,ackHandoff(deviceId,ack[1])?200:404,{ok:true})}
