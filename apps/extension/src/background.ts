...
/** Use a broad, purchasable search phrase for user preferences such as
 * "제로 또는 저당 아이스크림 바". Candidate validation still enforces the
 * matching alternative, package and quantity before anything can be added. */
function searchQueryFromItem(item:{rawText:string;productName?:unknown}){
 const productName=typeof item.productName==="string"&&item.productName.trim()?item.productName.trim():"";
 if(productName){
  const alternatives=productName.split(/\s+\uB610\uB294\s+/u,2);
  if(alternatives.length===2){
   const left=alternatives[0].trim().split(/\s+/u);const right=alternatives[1].trim().split(/\s+/u);
   if(left.length===1&&right.length>1)return `${left[0]} ${right.slice(1).join(" ")}`;
   return alternatives[0].trim();
  }
  return productName;
 }
 return item.rawText.replace(/^\s*\d{1,3}[.)]\s*/u,"").replace(/\s*\d+(?:\.\d+)?\s*(?:mL|ml|L|l|kg|g|mg|mcg|\u00b5g|IU|\uac1c|\ubcd1|\uce94|\ubd09|\ud1b5|\ud329|\ubc15\uc2a4|\ubb36\uc74c|\uc138\ud2b8|\uc815|\ucea1\uc290)\b/giu," ").replace(/[x\u00d7~\u2013]/gu," ").replace(/\s+/gu," ").trim();
}
...
chrome.runtime.onMessage.addListener((message,_sender,sendResponse)=>{
 ...
 if(message?.type==="DDAKDAMA_SEARCH_ALL"){if(!Array.isArray(message.items)||message.items.length<1||message.items.length>50){sendResponse({ok:false,error:"INVALID_ITEMS",output:[]});return false}(async()=>{let tabId:number|undefined;try{tabId=await createWorkerTab();const output=[];for(const item of message.items){if(typeof item.rawText!=="string"||item.rawText.length>500){output.push({requestLineId:String(item.id??""),results:[],error:"INVALID_ITEM"});continue}const searchQuery=searchQueryFromItem(item);const partner=await partnerSearch(searchQuery);let browser=[];let browserError="";try{browser=await searchOne(searchQuery,tabId)}catch(error){browserError=error instanceof Error?error.message:"SEARCH_FAILED"}const results=[...partner,...browser].filter((candidate,index,all)=>all.findIndex(other=>other.productId===candidate.productId&&other.vendorItemId===candidate.vendorItemId)===index);output.push({requestLineId:item.id,results,error:results.length?undefined:browserError||"NO_RESULTS"})}sendResponse({ok:true,output})}finally{await closeWorkerTab(tabId)}})().catch(error=>sendResponse({ok:false,error:error instanceof Error?error.message:"SEARCH_FAILED",output:[]}));return true}
 return false;
});
