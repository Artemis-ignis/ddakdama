...
type Job={id:string;productUrl:string;navigationUrl?:string;productId:string;vendorItemId:string|null;itemId:string|null;cartPurchaseQuantity:number;expectedUnitsPerPackage:number;expectedUnitSize:string|null;expectedStrength:string|null;expectedPackageContent:string|null;status:string};
type JobResult=Job&{status:string;beforeQuantity?:number;afterQuantity?:number;verifiedPrice?:number};
...
const validJob=(job:unknown):job is Job=>{const value=job as Partial<Job>;return Boolean(value&&typeof value.id==="string"&&value.id.length<=100&&typeof value.productUrl==="string"&&value.productUrl.length<=2000&&typeof value.productId==="string"&&/^\d+$/.test(value.productId)&&Number.isInteger(value.cartPurchaseQuantity)&&Number(value.cartPurchaseQuantity)>=1&&Number(value.cartPurchaseQuantity)<=20&&Number.isInteger(value.expectedUnitsPerPackage)&&Number(value.expectedUnitsPerPackage)>=1&&Number(value.expectedUnitsPerPackage)<=20)};
...
const quantityOf=(snapshot:{items?:Array<{productId:string;vendorItemId:string|null;itemId:string|null;quantity:number}>},job:Job)=>snapshot.items?.find(item=>item.productId===job.productId&&(!job.vendorItemId||item.vendorItemId===job.vendorItemId)&&(!job.itemId||item.itemId===job.itemId))?.quantity??0;
...
async function runJob(job:Job,checkpoint:CartCheckpoint|undefined,persist:(value:CartCheckpoint)=>Promise<void>){
 ...
 try{
  ...
  const after=quantityOf(await cartSnapshot(),job);
  return{...job,status:after===resume.targetQuantity?"SUCCESS":"QUANTITY_MISMATCH",beforeQuantity:resume.beforeQuantity,afterQuantity:after,verifiedPrice:detail.price??undefined};
...
async function deviceToken(){const stored=await chrome.storage.local.get("ddakdama-device-token");return String(stored["ddakdama-device-token"]??"")}
