...
chrome.runtime.onMessage.addListener((message,_sender,sendResponse)=>{
 ...
 if(message?.type==="DDAKDAMA_SEARCH_ALL"){if(!Array.isArray(message.items)||message.items.length<1||message.items.length>50){sendResponse({ok:false,error:"INVALID_ITEMS",output:[]});return false}(async()=>{let tabId:number|undefined;try{tabId=await createWorkerTab();const output=[];for(const item of message.items){if(typeof item.rawText!=="string"||item.rawText.length>500){output.push({requestLineId:String(item.id??""),results:[],error:"INVALID_ITEM"});continue}let queryPlan:ReturnType<typeof searchQueriesFromItem>=[];try{queryPlan=searchQueriesFromItem(item)}catch{output.push({requestLineId:String(item.id??""),results:[],error:"INVALID_PRODUCT_NAME"});continue}if(!queryPlan.length){output.push({requestLineId:String(item.id??""),results:[],error:"INVALID_PRODUCT_NAME"});continue}let results:unknown[]=[];let browserError="";const attempts=[] as Array<{stage:string;query:string;browserResultCount:number;partnerResultCount:number;error?:string}>;for(const {stage,query} of queryPlan){const partner=await partnerSearch(query);let browser:unknown[]=[];let error="";try{browser=await searchOne(query,tabId)}catch(caught){error=caught instanceof Error?caught.message:"SEARCH_FAILED";browserError=error}attempts.push({stage,query,browserResultCount:browser.length,partnerResultCount:partner.length,error:error||undefined});results=[...results,...partner,...browser].filter((candidate,index,all)=>{const value=candidate as {productId?:string;vendorItemId?:string|null};return all.findIndex(other=>{const otherValue=other as {productId?:string;vendorItemId?:string|null};return otherValue.productId===value.productId&&otherValue.vendorItemId===value.vendorItemId})===index});if(results.length)break}const group={requestLineId:item.id,results,error:results.length?undefined:browserError||"NO_RESULTS",searchedQueries:queryPlan.map(({query})=>query)};output.push(group);await recordSearchTrace({at:Date.now(),requestLineId:item.id,productName:item.productName??null,attempts,totalCandidateCount:results.length}).catch(()=>{})}sendResponse({ok:true,output})}finally{await closeWorkerTab(tabId)}})().catch(error=>sendResponse({ok:false,error:error instanceof Error?error.message:"SEARCH_FAILED",output:[]}));return true}
 return false;
});
