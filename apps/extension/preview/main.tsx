...
const success=lines.map((line,index)=>({id:line.id,status:"SUCCESS",beforeQuantity:0,afterQuantity:line.requestedPhysicalUnits/(index===1||index===2?2:1),verifiedPrice:prices[index],cartPrice:prices[index],expectedSubtotal:prices[index]*(line.requestedPhysicalUnits/(index===1||index===2?2:1)),cartAddedSubtotal:prices[index]*(line.requestedPhysicalUnits/(index===1||index===2?2:1))}));
