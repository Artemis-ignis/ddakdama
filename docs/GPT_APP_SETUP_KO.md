...
2. `https://ddakdama.artemis-clunk.workers.dev/health`가 `ok: true`를 반환하는지 확인합니다.
3. MCP 클라이언트로 `https://ddakdama.artemis-clunk.workers.dev/mcp`의 도구와 위젯을 검증합니다.
4. ChatGPT Developer Mode의 앱/플러그인 관리에서 새 앱을 만듭니다.
5. 연결 방식은 `Server URL`, 인증은 현재 공개 베타에서 `인증 없음`을 선택합니다.
6. Server URL에는 `https://ddakdama.artemis-clunk.workers.dev/mcp`를 입력합니다.
...
- 사용자 지원: `https://ddakdama.artemis-clunk.workers.dev/support`
- 앱 아이콘과 스크린샷
