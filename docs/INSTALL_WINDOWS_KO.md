...
사용자가 선택할 매니페스트는 `apps\extension\manifest.json` 하나입니다. `apps\extension\dist`는 `background.js`, `content.js`, `index.html`만 담는 빌드 폴더이므로 따로 선택하지 않습니다. 배포 ZIP을 받았다면 `ddakdama-extension-v1.0.0.zip`을 압축 해제하고, 그 안에서 `manifest.json`이 바로 보이는 최상위 폴더를 선택합니다.
